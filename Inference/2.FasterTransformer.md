...
FasterTransformer是Nvida推出的用于实现基于Transformer的神经网络推理的加速引擎。它包含Transformer块的高度优化版本的实现，其中包含编码器和解码器部分。
可以运行编码器-解码器架构模型（如：T5）、仅编码器架构模型（如：BERT）和仅解码器架构模型（如： GPT）的推理。

**Nvida鼓励所有开发人员利用 TensorRT-LLM 来获得 LLM。NVIDIA/FasterTransformer 存储库将保持运行，但不会进一步开发**
...
![](./FasterTransformer/image_bW89N6VXxa.png)
...
![](./FasterTransformer/image_RP616Yf0XC.png)
